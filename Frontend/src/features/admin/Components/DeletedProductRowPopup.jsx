...
const getStatusColor = (status) => {
  switch (status) {
    ...
    case 'Ended':
      return '#00b20fff'; // green
    default:
      return '#9766ff'; // Default purple
  }
};
    ...
    <Box sx={{ 
      ...
      <Box sx={{ 
        ...
        <Box sx={{ 
          ...
          {selectedProduct && (
            <>
              ...
              <DetailItem label="Category" value={selectedProduct.category || 'N/A'} />
              <Box sx={{ 
                            display: 'flex', 
                            alignItems: 'center', 
                            mb: 1,
        ...
        <Box sx={{ 
          ...
          />
          {selectedProduct.deletedBy && (
            <DetailItem 
              label="Deleted By" 
              value={`${selectedProduct.deletedBy}${selectedProduct.deletedByRole ? ` (${formatRoleName(selectedProduct.deletedByRole)})` : ''}`} 
            />
          )}
          ...
          <DetailItem 
            label="Company" 
            value={selectedProduct.company || 'N/A'} 
          />
          {selectedProduct.createdBy && (
            <DetailItem 
              label="Created By" 
              value={`${selectedProduct.createdBy}${selectedProduct.createdByRole ? ` (${formatRoleName(selectedProduct.createdByRole)})` : ''}`} 
            />
          )}

          {selectedProduct.editHistory && selectedProduct.editHistory.length > 0 && (
            <Box sx={{ mt: 1 }}>
              <Typography variant="subtitle1" gutterBottom sx={{ fontWeight: 'bold' }}>
                Edit History:
              </Typography>
              {selectedProduct.editHistory.map((edit, index) => (
                <Typography key={index} variant="body2" sx={{ ml: 2, mb: 0.5, color: '#555' }}>
                  • {edit.username} ({formatRoleName(edit.role)}) — {formatDate(edit.editedAt)}
                </Typography>
              ))}
            </Box>
          )}
