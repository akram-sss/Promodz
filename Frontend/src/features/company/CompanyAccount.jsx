...
const AdminAccount = () => {
  ...
  const fetchUserData = async () => {
    ...
    try {
      ...
      setFormData({
        ...
        followers: user._count?.followers ?? user.followers ?? '',
        rc: user.rc || '',
        email: user.email || '',
  ...
  const handleCancel = () => {
    ...
    if (originalData) {
      setFormData({
        ...
        name: originalData.fullName || originalData.name || '',
        followers: originalData._count?.followers ?? originalData.followers ?? '',
        rc: originalData.rc || '',
        ...
        role: originalData.role || 'ENTREPRISE'
      });
  ...
  const handleDelete = async () => {
    ...
    if (!password) return;
    if (!window.confirm('Are you sure you want to delete your company account? This action cannot be undone.')) return;
    ...
    try {
      ...
      setError(error.response?.data?.error || 'Failed to delete company account');
    } finally {
      setIsLoading(false);
    }
  };
        ...
        <Box sx={{ 
          ...
          <Box sx={{ 
            ...
            <Typography variant="body1" sx={{ color: "black" }}>
              <strong style={{ color: COLORS.primary }}>{formData.followers}</strong> followers
            </Typography>
          </Box>
        </Box>
          ...
          <Box sx={{ 
            ...
            >
              ...
              <Box sx={{ display: 'flex', justifyContent: 'space-between', mt: 2 }}>
                <PrimaryButton 
                  onClick={()=> navigate('/company/PasswordReset')}
                >
                  Forget Password ?
                </PrimaryButton>
                <PrimaryButton 
